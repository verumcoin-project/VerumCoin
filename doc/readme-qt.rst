VerumCoin-qt: Qt4 GUI for VerumCoin
===============================
...
Alternatively, install Qt Creator and open the `VerumCoin-qt.pro` file.

An executable named `VerumCoin-qt` will be built.
...
A warning for people using the *static binary* version of VerumCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of VerumCoin is linked against libdb5.3.
...
installed causes VerumCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
