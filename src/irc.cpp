...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("VerumCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #VerumCoinTEST2\r");
            Send(hSocket, "WHO #VerumCoinTEST2\r");
        } else {
            // randomly join #VerumCoin00-#VerumCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #VerumCoin-vmc-%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #VerumCoin-vmc-%02d\r", channel_number).c_str());
        }
